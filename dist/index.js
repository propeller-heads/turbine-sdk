...
var MOCKED_TURBINE_POOL = {
  metadata: {
    token0: getAddress("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"),
    // USDC
    token1: getAddress("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2"),
    // WETH
    fee: 3e3,
...
// src/models.ts
import { getAddress as getAddress2, parseUnits, formatUnits } from "viem";

// src/validation.ts
import { isAddress, isHex, hexToBytes, bytesToHex } from "viem";
...
var Token = class {
  constructor(address, decimals, symbol) {
    ...
    this.address = getAddress2(validatedAddress);
...
import { PERMIT2_ADDRESS as PERMIT2_ADDRESS2 } from "@uniswap/permit2-sdk";
  ...
  try {
    ...
    const permitSignature = await walletClient.signTypedData({
      ...
      domain: {
        name: "Permit2",
        chainId,
        verifyingContract: PERMIT2_ADDRESS2
      },
...
var _TurbineClient = class _TurbineClient {
  ...
  async addOrder(intent) {
    ...
    if (getAddress3(address) !== getAddress3(intent.owner)) {
      throw new TurbineError(
        "UNAUTHORIZED",
  ...
  async addOrders(intents) {
    ...
    const address = await this.ensureAuthenticated();
    if (intents.some((intent) => getAddress3(intent.owner) !== getAddress3(address))) {
      throw new TurbineError(
        "UNAUTHORIZED",
  ...
  async removeLiquidity(intent) {
    ...
    const address = await this.ensureAuthenticated();
    if (getAddress3(intent.owner) !== getAddress3(address)) {
      throw new TurbineError(
        "UNAUTHORIZED",
  ...
  async addLiquidityWithSignedPermit(payload) {
    ...
    const address = await this.ensureAuthenticated();
    if (getAddress3(payload.addLiquidity.owner) !== getAddress3(address)) {
      throw new TurbineError(
        "UNAUTHORIZED",
        "Authenticated user does not match the intent owner."
      );
    ...
    try {
      ...
      if (err instanceof BaseError) {
        const revertError = err.walk(
          (err2) => err2 instanceof ContractFunctionRevertedError
        );
  ...
  parseSignature(signature) {
    ...
    const rHex = bytesToHex2(r);
    const sHex = bytesToHex2(s);
...
async function getPools(publicClient, hookAddress) {
  try {
    ...
    return poolsData.map(
      (poolData) => ({
        metadata: {
          token0: getAddress3(poolData.token0),
          token1: getAddress3(poolData.token1),
          fee: poolData.fee,
          lpToken: getAddress3(poolData.lpToken)
        },
        state: {
...
function getRandomSalt() {
  ...
  return bytesToHex2(randomBytes2);
}
